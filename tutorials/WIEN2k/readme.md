...
## Troubleshooting
- If the RMTs overlap, an error will be displayed. Therefore, you need to set a larger RMT reduction rate. Increase the number immediately after "-red" in "ElaStic_WIEN2k_init.sh" shown below (the example below shows changing from the initial "-red 8.0" to "-red 9.0"). It is better to increase the value as long as it does not overlap, but since adding distortion makes it difficult to predict, a slightly larger value may be fine.
```
WIEN2k_init_lapw_silent.csh -b -red 9.0 -vxc 13  -ecut -10.0  -in1_rkmax 8.0  -in2_method GAUSS  -in2_gmax 14  -in2_smear 0.01  -mix 0.1  -kgen_numk 2500  -kgen_shift 0  -inM_method PORT  -inM_tolf 0.1
```
