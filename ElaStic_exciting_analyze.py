    ...
    for j in range(1, NoP+1):
        ...
        if (os.path.exists(Dstn_num)): 
            ...
            if os.path.exists('TOTENERGY.OUT'):
                with open('TOTENERGY.OUT', 'r') as e_file:
                    enrgis = [line.strip() for line in e_file if line.strip()]
...
try:
    from numpy.lib.polynomial import RankWarning  # NumPy >= 1.24
except ImportError:
    from numpy import RankWarning  # NumPy <= 1.23
...
for i in range(1, ECs+1):
    if (i<10):
        ...
        Dstn = 'Dst' +str(i)
    print(f"{Dstn}    eta_max    Fit_order", file=fri)
fri.close()
#--------------------------------------------------------------------------------------------------
